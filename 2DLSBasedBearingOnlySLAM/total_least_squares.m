...
function [XR, XL, chi_stats_l, num_inliers_l, chi_stats_r, num_inliers_r, H, b] = doTotalLS(XR, XL,
	     BZl, landmark_associations,
  ...
  for (iteration=1:num_iterations)
    ...
    if (num_landmarks) 
      [H_landmarks, b_landmarks, chi_, num_inliers_] = linearizeLandmarks(XR, XL, BZl, landmark_associations,num_poses, num_landmarks, kernel_threshold_bearings);
      chi_stats_l(iteration)=chi_;
      num_inliers_l(iteration)=num_inliers_;
    endif;

    [H_poses, b_poses, chi_, num_inliers_] = linearizePoses(XR, XL, Zr, pose_associations,num_poses, num_landmarks, kernel_threshold_poses);
